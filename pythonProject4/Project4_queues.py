import math
from numbers import Number


def is_valid(lamda: float, mu: float, c: int = 1) -> bool:
    """
    Boolean function determining if the given queue is valid
    :param lamda: arrival rate
    :param mu: service rate
    :param c: amount of servers
    :return: True if the queue is valid, false otherwise
    """

    # Checks if lamda is a tuple or scalar
    if isinstance(lamda, tuple):
        # Lamda is a tuple, so each value of lamda is tested for validity
        for i in lamda:
            if not isinstance(i, Number) or i <= 0:
                return False

    else:
        # Lamda is a scalar, test for validity
        if not isinstance(lamda, Number) or lamda <= 0:
            return False

    # Tests mu for validity
    if not isinstance(mu, Number) or mu <= 0:
        return False

    # Tests c for validity
    if not isinstance(c, Number) or c <= 0:
        return False

    # If the inputs have passed all tests, the queue is valid
    return True


def is_feasible(lamda: float, mu: float, c: int = 1) -> bool:
    """
    Boolean function determining if the given queue is feasible
    :param lamda: arrival rate
    :param mu: service rate
    :param c: amount of servers
    :return: True if the queue is feasible, false otherwise
    """

    # First tests to make sure queue is valid
    if not is_valid(lamda, mu, c):
        return False

    # Checks if lamda is a tuple or scalar
    if isinstance(lamda, tuple):
        # Lamda is a tuple, so we use lamda aggregate in our feasibility equation
        wlamda = sum(lamda)

    else:
        wlamda = lamda

    # Calculates p
    p = wlamda / (c * mu)

    # Test feasibility
    if not p < 1:
        return False

    # If queue passes all tests, it is feasible
    return True


def calc_p0(lamda: float, mu: float, c: int = 1) -> float:
    """
     Calculates p0, the probability of an empty queue for single and multiple
     server queues
     :param lamda: arrival rate
     :param mu: service rate
     :param c: amount of servers
     :return: p0, probability of an empty queue
     """

    # Runs through basic checks for validity and feasibility
    if is_valid(lamda, mu, c) is False:
        return math.nan

    if is_feasible(lamda, mu, c) is False:
        return math.inf

    # If lamda is a tuple, we must use lamda aggregate for our p0 calculation
    if isinstance(lamda, tuple):
        wlamda = sum(lamda)

    else:
        wlamda = lamda

    # Prepare r and p for p0 calculation later on
    r = wlamda / mu
    p = r / c

    # Calculate p0 if there are multiple servers
    if c > 1:

        list_for_p0 = []

        for n in range(0, c):
            term1 = (r ** n) / math.factorial(n)
            list_for_p0.append(term1)

        term2 = (r ** c) / (math.factorial(c) * (1 - p))

        p0 = 1 / (sum(list_for_p0) + term2)

    # Calculates p0 if there is a single server
    else:
        p0 = 1 - p

    return p0


def calc_lq_mmc(lamda: float, mu: float, c: int = 1) -> float:
    """
     Calculates lq, the average number of customers waiting for an m/m/c queue
     :param lamda: arrival rate
     :param mu: service rate
     :param c: amount of servers
     :return: lq, the average number of customers waiting for an m/m/c queue
     """

    # Calculates p0
    p0 = calc_p0(lamda, mu, c)

    # If the queue did not pass validity or feasibilty test, return the error value
    if math.isinf(p0) or math.isnan(p0):
        return p0

    # If lamda is a tuple, we must use lamda aggregate for our lq calculation
    if isinstance(lamda, tuple):
        wlamda = sum(lamda)

    else:
        wlamda = lamda

    # Calculate r and p values for later lq calculation
    r = wlamda / mu
    p = r / c

    # Calculate the two terms in lq equation for multiple server queues
    if c > 1:
        lq_num = r ** c * p * p0
        lq_denom = math.factorial(c) * (1 - p) ** 2

    # Calculate the two terms in lq equation for single server queues
    else:
        lq_num = wlamda ** 2
        lq_denom = mu * (mu - wlamda)

    # Calculate lq
    lq = lq_num / lq_denom

    return lq


def calc_bk_mmc(k: int, lamda: tuple, mu: float, c: int = 1) -> float:
    """
    :param k: priority class
    :param lamda: arrival rate
    :param mu: service rate
    :param c: number of servers default is 1
    :return: calculates bk for a given class k for a m/m/c queue where bk is capacity remaining for classes
    that are lower priority than k
    """

    # Check if queue is valid
    if not is_valid(lamda, mu, c):
        return math.nan

    # Check if queue feasible
    if not is_feasible(lamda, mu, c):
        return math.inf

    # If lamda is not a tuple, turn it into one
    if not isinstance(lamda, tuple):
        a = [lamda]
        lamda = tuple(a)

    # Check that k is a valid input
    if not isinstance(k, int) or k < 0 or k > len(lamda):
        return math.nan

    # Bk = 1 if k = 0
    if k == 0:
        return 1

    # Calculate Bk
    p = [a / (c * mu) for a in lamda[0:k]]

    bk = 1 - sum(p)

    return bk


def calc_wqk_mmc(k: int, lamda: float, mu: float, c: int = 1) -> float:
    """

    :param k: priority class
    :param lamda: arrival rate
    :param mu: service rate
    :param c: number of servers default is 1
    :return: calculates the average waiting time, wq_k for a customer of priority class k for a m/m/c queue
    """

    # Calculate lq, if inputs are not valid return error message
    lq = calc_lq_mmc(lamda, mu, c)

    if lq == math.inf or lq == math.nan:
        return lq

    # if lamda is a tuple, we need to calculate lamda aggregate. If it is not a tuple, turn it into a tuple
    # and lamda aggregate = lamda
    if isinstance(lamda, tuple):
        wlamda = sum(lamda)

    else:
        wlamda = lamda
        a = [lamda]
        lamda = tuple(a)

    # Check that k is a valid input
    if not isinstance(k, int) or k <= 0 or k > len(lamda):
        return math.nan

    # ro calculation
    p = wlamda / (c * mu)

    # bk calculation
    bk = calc_bk_mmc(k-1, lamda, mu, c)

    # Second bk calculation
    bk_2 = calc_bk_mmc(k, lamda, mu, c)

    # wqk calculation
    wqk = ((1 - p) * lq) / (wlamda * bk * bk_2)

    return wqk


def calc_lqk_mmc(k: int, lamda: tuple, wqk: float) -> float:
    """
    Calculates Lq,k for a customer of priority class k for a m/m/c queue
    :param k: priority class
    :param lamda: arrival rate(s)
    :param wqk: average waitiing time for a customer of class k
    :return: Lq,k is average number of customers of priority class k that are waiting
    """

    # If lamda is not a tuple, change it to one
    if not isinstance(lamda, tuple):
        a = [lamda]
        lamda = tuple(a)

    # Checks that k is a valid priority class
    if not isinstance(k, int) or k <= 0 or k > len(lamda):
        return math.nan

    # Calculate and return lqk
    lamdak = lamda[k-1]

    lqk = lamdak * wqk

    return lqk


def use_littles_law(lamda: float, mu: float, c: int = 1, **kwargs) -> dict:
    """
    Returns a dictionary containing all common metrics for a queue
    :param lamda: arrival rate(s)
    :param mu: service rate
    :param c: amount of servers
    :param kwargs: User inputted metrics
    :return: metric_dict, a dictionary containing all common metrics for a queue
    """

    # if kwargs is empty, return math.nan
    if len(kwargs) == 0:
        return math.nan

    # Retrieve key and element from kwargs
    list_keys = list(kwargs.keys())
    first_key = list_keys[0]
    first_element = kwargs[first_key]
    first_key = first_key.lower()

    # If lamda is a tuple, get aggregate value of lamda
    if isinstance(lamda, tuple):
        wlamda = sum(lamda)

    else:
        wlamda = lamda

    # From kwargs, calculate lq
    if first_key == 'lq':
        lq = first_element

    elif first_key == 'l':
        lq = first_element - wlamda / mu

    elif first_key == 'wq':
        lq = first_element * wlamda

    elif first_key == 'w':
        lq = (first_element * wlamda) - wlamda/mu

    else:
        return math.nan

    # Calculate the metrics
    r = wlamda / mu
    p = r / c
    l = lq + r
    wq = lq / wlamda
    w = l / wlamda

    metric_dict = {'lq': lq, 'l': l, 'wq': wq, 'w': w, 'r': r, 'p': p}

    # If lamda is a tuple, calculate additional metrics
    if isinstance(lamda, tuple):

        wqk = [calc_wqk_mmc(k+1, lamda, mu, c) for k in range(0, len(lamda))]

        lqk = [calc_lqk_mmc(k+1, lamda, wqk[k]) for k in range(0, len(lamda))]

        tuple_metrics = {'wqk': tuple(wqk), 'lqk': tuple(lqk)}
        metric_dict.update(tuple_metrics)

    return metric_dict
